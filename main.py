...
class KmeansClustering:
    ...
    def choosefile(self):
        ...
        if( not (self.file_path[-5:]==".xlsx" or self.file_path[-4:]==".xls")):
            tkMessageBox.showerror("K Means Clustering", "insert an excel file")
            return

        self.df = pd.read_excel(self.file_path)

        if self.df.empty:
            tkMessageBox.showerror("K Means Clustering", "invalid file!")
            return
    ...
    def validate(self, cluster, init, path):
        if  (path==""):  # the field is being cleared
            tkMessageBox.showerror("K Means Clustering", "Please enter path")
            return False
        if not cluster and not init:  # the field is being cleared
            tkMessageBox.showerror("K Means Clustering", "Please enter values")
            return False
        if not cluster:  # the field is being cleared
            tkMessageBox.showerror("K Means Clustering", "Please enter number of clusters")
            return False
        if not init:  # the field is being cleared
            tkMessageBox.showerror("K Means Clustering", "Please enter number of runs")
            return False
        try:
            ...
            if self.n_clusters < 1:
                tkMessageBox.showerror("K Means Clustering", "The number for clusters should be bigger than 0")
                return False
            elif self.n_clusters > len(self.df.columns):
                tkMessageBox.showerror("K Means Clustering", "Num of clusters shouldn't be higher then the number of columns")
                return False
            elif self.n_init < 1:
                tkMessageBox.showerror("K Means Clustering", "The number for runs should be bigger than 0")
                return False
            else:
                return True
